...
text, the session timestamp is prefixed on every turn (not just a session's
first), and an image caption (when present) is appended without downloading
the image.
"""
...
def test_speaker_name_is_preserved_in_message_content() -> None:
    ...
    assert content == "[Session date: 1:56 pm on 8 May, 2023] Caroline: I started painting again."


def test_every_turn_is_prefixed_with_the_session_timestamp() -> None:
    turn = LocomoTurn(
        dia_id="D1:2",
        speaker="Bob",
        text="Nice!",
    ...
    content = _turn_content(turn)
    assert content.startswith("[Session date: 1:56 pm on 8 May, 2023]")
...
def test_image_caption_is_appended_without_downloading_the_image() -> None:
    ...
    content = _turn_content(turn)
    assert content == (
        "[Session date: 1:56 pm on 8 May, 2023] Caroline: Look at this! "
